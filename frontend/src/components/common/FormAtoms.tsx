...
export function FormLabel({ children, required, htmlFor }: { children: ReactNode; required?: boolean; htmlFor?: string }) {
  return (
    <label htmlFor={htmlFor} className="block text-xs font-medium text-slate-600 mb-1.5 ml-1">
      {children}
...
export function FormHelperText({ children, isError }: { children: ReactNode; isError?: boolean }) {
  return <p className={`mt-1.5 text-[10px] font-medium ml-1 leading-tight ${isError ? 'text-red-500' : 'text-slate-400/90'}`}>{children}</p>;
}
...
export function FormInput({ label, helper, error, required, className = '', id, ...props }: InputProps) {
  ...
  return (
    <div className="w-full">
      {label && <FormLabel required={required} htmlFor={inputId}>{label}</FormLabel>}
...
export function FormTextarea({ label, helper, error, required, className = '', id, ...props }: TextareaProps) {
  const textareaId = id || (label ? label.toLowerCase().replace(/\s+/g, '-') : undefined);
  const isInvalid = !!error;

  return (
    <div className="w-full">
      {label && <FormLabel required={required} htmlFor={textareaId}>{label}</FormLabel>}
      <textarea 
        id={textareaId}
...
export function FormSelect({ label, helper, error, required, children, className = '', id, ...props }: SelectProps) {
  const selectId = id || (label ? label.toLowerCase().replace(/\s+/g, '-') : undefined);
  const isInvalid = !!error;

  return (
    <div className="w-full">
      {label && <FormLabel required={required} htmlFor={selectId}>{label}</FormLabel>}
  ...
  return (
    <div className="flex flex-col gap-1 w-full">
      ...
      >
        ...
        <div className={`w-12 h-6 rounded-full transition-colors relative ${checked ? 'bg-indigo-600' : 'bg-slate-300'}`}>
